    ...
    public function traiter(string $methode, string $chemin, array $corps = []): array
    {
        ...
        if ($chemin === '/api/admin/produits' && $methode === 'POST') {
            ...
            $produit = [
                ...
                'descriptionLongue' => nettoyer_multiligne($corps['descriptionLongue'] ?? '', 4000),
                'specifications' => nettoyer_specifications($corps['specifications'] ?? []),
                'images' => nettoyer_images($corps['images'] ?? []),
